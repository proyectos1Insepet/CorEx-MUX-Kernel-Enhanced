...
/*******************************************************************************
* Function Name: EEPROM_Internal_Enable
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Internal_Start
********************************************************************************
*
* Summary:
*  Starts EEPROM.
...
{
    EEPROM_Internal_Enable();
}


/*******************************************************************************
* Function Name: EEPROM_Internal_Stop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Internal_WriteByte
********************************************************************************
*
* Summary:
...
*  call EEPROM_Internal_UpdateTemperature() function if the temperature of the
    ...
    if (address < CY_EEPROM_SIZE)
    {
        ...
        if(CYRET_SUCCESS == CySpcLock())
        {
            status = CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, byteNumber, &dataByte, \
                                                                    EEPROM_Internal_SPC_BYTE_WRITE_SIZE);
...
/*******************************************************************************
* Function Name: EEPROM_Internal_ReadByte
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Internal_UpdateTemperature
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EEPROM_Internal_EraseSector
********************************************************************************
*
* Summary:
...
cystatus EEPROM_Internal_EraseSector(uint8 sectorNumber) 
{
    ...
    if(sectorNumber < (uint8) EEPROM_Internal_SECTORS_NUMBER)
    {
...
/*******************************************************************************
* Function Name: EEPROM_Internal_Write
********************************************************************************
*
* Summary:
...
*  EEPROM_Internal_UpdateTemperature() function if the temperature of the
...
/*******************************************************************************
* Function Name: EEPROM_Internal_StartWrite
********************************************************************************
*
* Summary:
...
*  EEPROM_Internal_Query(). EEPROM_Internal_Query() must be called
*  until it returns a status other than CYRET_STARTED. That indicates that the
*  write has completed. Until EEPROM_Internal_Query() detects that
...
*  you should call EEPROM_Internal_UpdateTemperature() API if the temperature
...
/*******************************************************************************
* Function Name: EEPROM_Internal_StartErase
********************************************************************************
*
* Summary:
...
*  must be used in combination with EEPROM_Internal_Query().
*  EEPROM_Internal_Query() must be called until it returns a status
*  other than CYRET_STARTED. That indicates the erase has been completed.
*  Until EEPROM_Internal_Query() detects that the erase is
...
/*******************************************************************************
* Function Name: EEPROM_Internal_Query
********************************************************************************
*
* Summary:
*  Checks the status of an earlier call to EEPROM_Internal_StartWrite() or
*  EEPROM_Internal_StartErase().
...
/*******************************************************************************
* Function Name: EEPROM_Internal_ByteWritePos
********************************************************************************
*
* Summary:
    ...
    if((rowNumber < (uint8) CY_EEPROM_NUMBER_ROWS) && (byteNumber < (uint8) SIZEOF_EEPROM_ROW))
    {
        ...
        if(CySpcLock() == CYRET_SUCCESS)
        {
            ...
            if(CySpcLoadMultiByte(CY_SPC_FIRST_EE_ARRAYID, (uint16)byteNumber, &dataByte,\
                                                                EEPROM_Internal_SPC_BYTE_WRITE_SIZE) == CYRET_STARTED)
            {
                while(CY_SPC_BUSY)
                {
                    /* Wait until SPC becomes idle */
