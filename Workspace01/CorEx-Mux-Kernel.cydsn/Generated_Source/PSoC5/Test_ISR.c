...
void Test_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Test_ISR__INTC_NUMBER] = address;
}
...
cyisraddress Test_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Test_ISR__INTC_NUMBER];
}
...
{
    *Test_ISR_INTC_PRIOR = priority << 5;
}
...
uint8 Test_ISR_GetPriority(void)
{
    ...
    priority = *Test_ISR_INTC_PRIOR >> 5;
