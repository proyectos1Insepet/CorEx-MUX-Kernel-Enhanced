...
static uint8 XDATA I2C_Bus_slReadBuf[I2C_Bus_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_Bus_slWriteBuf[I2C_Bus_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  I2C_Bus_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_Bus_slReadBuf -  The global variable used to store a response.
*  I2C_Bus_slRdBufIndex - The global variable used to store a current
...
void I2C_Bus_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_Bus_SlaveInitWriteBuf(I2C_Bus_slWriteBuf, I2C_Bus_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_Bus_SlaveInitReadBuf (I2C_Bus_slReadBuf, 0u);
...
*
*******************************************************************************/
cystatus I2C_Bus_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
*
* Global variables:
*  I2C_Bus_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_Bus_slStatus & I2C_Bus_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_Bus_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_Bus_slWriteBuf,
                              I2C_Bus_MIN_UNT16(byteCount, size));
