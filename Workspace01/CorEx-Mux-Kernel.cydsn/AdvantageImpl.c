...
void AdvantageActivateProgramming(LPVOID pparam);
...
void AcquirePumpStateResponse(void *pparam)
{
    ...
    if(pjob)
    {
        ...
        if((pjob->_ppump->_pumpstate == PUMP_FEOT || pjob->_ppump->_pumpstate == PUMP_PEOT) && 
            (prevstate == PUMP_BUSY))
            pjob->_ppump->_pumprftransstate = RF_CASHSALEREPORT;
        /*if((pjob->_ppump->_pumpstate == PUMP_IDLE || pjob->_ppump->_pumpstate == PUMP_CALLING) && 
    ...
    {
        {
            ...
            {            
                ...
                if(pjob->_ppump->_trasactionbuffer[0x00] == 0x0D)
                {
                    ...
                    pjob->_ppump->_authorizationinfo._price = LSDBCDBUFF2HEX(PBYTECAST(&pjob->_ppump->_rxbuffer[0x0C]), 0x04);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            else if(_g_dispenserlayoutinfo._displaydigitsmode == 0x07)
            {
...
void PumpPresetNotification(LPVOID pparam)
{
    ...
    if(pjob)
    {
        ...
        if(pts)
        {
            ...
            //Entering here means that volume preset was chosen and since this is a 6-6-4 dispenser, this multiplication is required
            if(pjob->_ppump->_trasactionbuffer[0x0004] == 0x10)
            {
                PSTRBUFFTOGG(PBYTECAST(&pjob->_ppump->_trasactionbuffer[0x0008]), 0x08);
                uint32 volumeamount = LSDBCDBUFF2HEX(PBYTECAST(&pjob->_ppump->_trasactionbuffer[0x0008]), 0x08);
                memset(PBYTECAST(&pjob->_ppump->_trasactionbuffer[0x0008]), 0x00, 0x08);
                B78AD90CF552D9B30A(volumeamount * 10, PBYTECAST(&pjob->_ppump->_trasactionbuffer[0x0008]));
            }else//Money PRESET
                PSTRBUFFTOGG(PBYTECAST(&pjob->_ppump->_trasactionbuffer[0x0008]), 0x08);
...
void AcquireDispenserLayoutInformation(LPVOID pparam)
{
    ...
    else if(_g_dispenserlayoutinfo._numpositions == 0x04)
    {
        //PLEASE CHECK THE DOCUMENTATION TO UNDERSTAND THE COMPARISON VALUES
        switch(ptrbuff[0x00])
        {
            ...
            case 0x12://MPD with 3 grades/hoses per side
            {
                _g_dispenserlayoutinfo._hosesposition1 = 0x03;
                _g_dispenserlayoutinfo._hosesposition2 = 0x03;
                _g_dispenserlayoutinfo._hosesposition3 = 0x03;
                _g_dispenserlayoutinfo._hosesposition4 = 0x03;
                break;
            }
            case 0x13://MPD with 4 grades/hoses per side
            {
                _g_dispenserlayoutinfo._hosesposition1 = 0x04;
                _g_dispenserlayoutinfo._hosesposition2 = 0x04;
                _g_dispenserlayoutinfo._hosesposition3 = 0x04;
                _g_dispenserlayoutinfo._hosesposition4 = 0x04;
                break;
            }
            case 0x14://MPD with 5 grades/hoses per side
            {
                _g_dispenserlayoutinfo._hosesposition1 = 0x05;
                _g_dispenserlayoutinfo._hosesposition2 = 0x05;
                _g_dispenserlayoutinfo._hosesposition3 = 0x05;
                _g_dispenserlayoutinfo._hosesposition4 = 0x05;
                break;
            }
            case 0x15://MPD with 6 grades/hoses per side
            {
    ...
    if(pjob->_ppump->_datacontainer._var0 < 0xF)
    {
        ...
        if(pjob->_ppump->_datacontainer._var1 < _MAX_NUMBER_OF_PUMPS_)
        {
            ...
            if(_g_ptractprogramming == PNEAR_NULLPTR)
                _g_ptractprogramming = AdvantageActivateProgramming;
        }
